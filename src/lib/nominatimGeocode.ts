...
/**
 * 詳細（丁目・番地）が不明なとき用: 都道府県＋市、または郡＋町村、東京都は区、政令市は「市＋区」まで含める。
 */
export function municipalityLevelQuery(address: string): string | null {
  ...
  const prefAndCity = s.match(/^(.+?(?:北海道|[^、，]+?[都道府県]))(.+?市)/);
  if (prefAndCity) {
  ...
  const gun = s.match(/^(.+?(?:北海道|[^、，]+?[都道府県]))(.+?郡.+?[町村])/);
  if (gun) return gun[1] + gun[2];

  const townOnly = s.match(/^(.+?(?:北海道|[^、，]+?[都道府県]))([^、，市郡]*?[町村])/);
...
export async function geocodeJapaneseAddress(fullAddress: string): Promise<[number, number] | null> {
  ...
  if (muni && muni !== q && !tryQueries.includes(muni)) tryQueries.push(muni);
